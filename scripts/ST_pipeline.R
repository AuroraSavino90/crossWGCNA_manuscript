...
#####################################################################
####################### define epi and stroma spots, smooth gene expression, filter the spots and merge stroma and epi data
#####################################################################
...
save(averaged_expr_all_fun, file="averaged_expr_all_fun.RData")
...
adj<-Adjacency(data=md[[1]], method="netdiff", Adj_type=at, cortype=ct, pval="none", thr=0.05, beta=b, comp1="_tis1", comp2="_tis2")
save(adj, file="ST_adj_pipeline.RData")

net<-crossWGCNA(data=md[[1]], method="netdiff", doClusters = T, Adj_type=at, cortype=ct, pval="none", thr=0.05, beta=b, comp1="_tis1", comp2="_tis2")
save(net, file="ST_net_pipeline.RData")
...
write.xlsx(data.frame(fgseaRes1), file="ST_GSEA_epi_ratio2_s.xlsx")
...
write.xlsx(data.frame(fgseaRes2), file="ST_GSEA_stroma_ratio2_s.xlsx")

#####################################################################
####################### select and represent the most connected gene pair
#####################################################################
...
pdf(paste(gene1, gene2, "scatter.pdf", sep="_"),5,5)
...
pdf(paste(gene1, gene2, "comm.pdf", sep="_"),6,5)
print(p)
dev.off()
...
pdf(paste(gene1, "expr.pdf", sep="_"),6,5)
print(p1)
dev.off()

pdf(paste(gene2, "expr.pdf", sep="_"),6,5)
print(p2)
dev.off()
...
pdf(paste(gene1, "origVSave.pdf", sep="_"),9,5)
...
pdf(paste(gene2, "origVSave.pdf", sep="_"),9,5)
ggplot(data=df, aes(x=x_coord, y=y_coord, colour=gene))+geom_point()+
        scale_color_continuous(type = "viridis")+facet_grid(~type)+geom_point(data=df_midpoint, size=1, colour="black")+theme_classic()
dev.off()

#####################################################################
####################### represent correlations between gene pairs
#####################################################################

pdf("ST_COL6A2_EFNA1_cor_inspect_s.pdf", 6, 6)
cor_inspect(md[[1]], gene1, gene2)
dev.off()

pdf("ST_HLA-DRB1_ACTG1_cor_inspect_s.pdf", 6, 6)
cor_inspect(md[[1]], gene1="ACTG1", gene2="HLA-DRB1")
dev.off()

pdf("ST_LGALS9_ACTG1_cor_inspect_s.pdf", 6, 6)
...
pdf("ST_COL6A2_top10_corrs_s.pdf", 8,7 )
...
pdf("ST_mod1_tis1.pdf", 6,5)
p1
dev.off()

pdf("ST_mod1_tis2.pdf", 6,5)
p2
dev.off()

pdf("ST_mod1_corr.pdf", 5,5)
plot(wm[[1]][sp[[1]]], wm[[2]][sp[[2]]],pch=19, xlab="weighted expression comp1", ylab="weighted expression comp2")
dev.off()

pdf("ST_mod1_comm.pdf", 6,5)
